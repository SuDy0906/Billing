...
void shopping :: menu(){
    ...
    switch(choice){
        case 1:{
            ...
            if(email=="sudysingh@gmail.com" && password=="sudy123"){
                admin();
            }
            else{
...
void shopping :: add(){
	...
	if(token==1) //if code already exist the admin will have to re enter
	 goto m;
	else{ //unique
...
void shopping ::edit()
{
	...
	if(!data) cout<<"\n\nFile doesn't exist! ";
		...
		while(!data.eof()){
			...
			else data1<<" "<<pcode<<" "<<pname<<" "<<price<<" "<<dis<<"\n";
...
void shopping::rem()
{
	...
	if(!data) cout<<"File doesnt exist";
